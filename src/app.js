...
let worker=null,config=PNG2ANSI.configFrom(),sourceFile=null,sourceUrl=null,resultUrls=[],refs=[],job=0,timer=0,lastResult=null;
...
const advanced=[
  ...
  {path:"nl_filter.enabled",label:"Enable NL Filter",type:"checkbox",hint:"Runs after Derez and before tonal controls."},
...
function controlMarkup(item){
  const value=getPath(item.path),id=`field-${item.path.replaceAll(".","-")}`,hint=item.hint?`<small class="field-hint">${escapeHtml(item.hint)}</small>`:"";
  if(item.type==="select")return `<div class="field"><label for="${id}">${item.label}</label><select id="${id}" data-path="${item.path}">${item.options.map(option=>`<option${option===value?" selected":""}>${option}</option>`).join("")}</select>${hint}</div>`;
  if(item.type==="checkbox")return `<div class="field checkbox-field"><label for="${id}"><span>${item.label}</span><input id="${id}" data-path="${item.path}" type="checkbox"${value?" checked":""}></label>${hint}</div>`;
  return `<div class="field"><label for="${id}"><span>${item.label}</span><output>${value}</output></label><input id="${id}" data-path="${item.path}" type="${item.range?"range":"number"}" min="${item.min}" max="${item.max}" step="${item.step}" value="${value}">${hint}</div>`;
}
...
function syncControl(input,item,value){if(item.type==="checkbox")input.checked=value;else input.value=value;const output=input.closest(".field")?.querySelector("output");if(output)output.value=value;}
...
function renderControls(){
  ...
  document.querySelectorAll("[data-path]").forEach(input=>{const item=controls.find(control=>control.path===input.dataset.path),event=item.range?"input":"change";input.addEventListener(event,()=>commitControl(input,item));if(!item.range&&item.type!=="select"&&item.type!=="checkbox"){input.addEventListener("blur",()=>commitControl(input,item));input.addEventListener("keydown",event=>{if(event.key==="Enter")commitControl(input,item);});input.addEventListener("input",()=>{const value=Number(input.value);if(input.value!==""&&Number.isFinite(value)&&(value<item.min||value>item.max))commitControl(input,item);});}});
...
function workloadText(units){return `${(units/1000000).toFixed(1)}M / ${(PNG2ANSI.MAX_WORK_UNITS/1000000).toFixed(0)}M work units`;}
function updateWorkload(){const units=PNG2ANSI.workloadUnits(config,currentCodes().length),host=$("#workload-estimate");host.textContent=workloadText(units);host.classList.toggle("over-budget",units>PNG2ANSI.MAX_WORK_UNITS);return units;}
...
function schedule(notice=""){
  ...
  const id=++job;stopWorker();lastResult=null;toggleDownloads(false);$("#progress").style.width="1%";
  try{PNG2ANSI.validateWorkload(config,currentCodes().length);}catch(error){setStatus(error.message,"error");$("#progress").style.width="0";return;}
  setStatus(`${notice}Queued latest settings…`,"busy");timer=setTimeout(()=>convert(id),240);
}
async function convert(id){
  if(id!==job)return;setStatus("Preparing worker…","busy");$("#progress").style.width="2%";
  try{const bitmap=await createImageBitmap(sourceFile);if(id!==job){bitmap.close();return;}worker=makeWorker(id);workerStats.started++;worker.postMessage({id,bitmap,config:PNG2ANSI.clone(config),codes:currentCodes()},[bitmap]);}
...
window.__PNG2ANSI_TEST__={offline:true,version:"0.2.0",defaults:PNG2ANSI.clone(PNG2ANSI.DEFAULTS),loadSource,schedule,get config(){return PNG2ANSI.clone(config);},get references(){return refs.length;},get job(){return job;},get workerStats(){return{...workerStats};}};
