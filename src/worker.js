...
self.onmessage=async event=>{
  const {id,bitmap,config,codes,maxWorkUnits,overrideWorkLimit}=event.data; latest=Math.max(latest,id);
  try{
    const result=await PNG2ANSI.convert(bitmap,config,codes,value=>{if(id===latest)self.postMessage({id,type:"progress",value});},{maxWorkUnits,overrideWorkLimit});
    if(id!==latest)return;
